...
export function getPregameTeamScope(game) {
  ...
  const fallbackTeamId = String(game?.homeTeam?.teamId || game?.awayTeam?.teamId || "").trim();
  return fallbackTeamId ? `team:${fallbackTeamId}` : null;
}
...
export function getPregameTeamScopeForTeam(team, game = null) {
  ...
  const fallbackTeamId = String(team?.teamId || "").trim();
  return fallbackTeamId ? `team:${fallbackTeamId}` : null;
}
...
export function getTeamBoxScorePlayers(game, teamScope) {
  ...
  if (teamScope.startsWith("team:")) {
    ...
    if (String(game?.homeTeam?.teamId || "") === scopeTeamId) {
      return Array.isArray(game?.boxScore?.home?.players) ? game.boxScore.home.players : [];
    }
    if (String(game?.awayTeam?.teamId || "") === scopeTeamId) {
      return Array.isArray(game?.boxScore?.away?.players) ? game.boxScore.away.players : [];
    }
    return [];
  }
