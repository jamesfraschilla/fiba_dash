...
export default function Game({ variant = "full" }) {
  ...
  const regulationPeriodSeconds = game?.regulationPeriodSeconds || 10 * 60;
  ...
  const trackedFoulPeriods = (period) => {
    if (period <= 2) return [1, 2];
    if (period === 3) return [3];
    return [4, ...Array.from({ length: Math.max(0, period - 4) }, (_, index) => 5 + index)];
  };
  const teamFoulInfo = (teamId) => {
    let count = 0;
    const trackedPeriods = new Set(trackedFoulPeriods(currentPeriod));
    (game?.playByPlayActions || []).forEach((action) => {
      if (!trackedPeriods.has(action.period)) return;
      if (!isTeamFoulAction(action)) return;
      if (action.teamId !== teamId) return;
      count += 1;
    });
    return { count: Math.min(count, foulLimit), inPenalty: count >= foulLimit };
  };
  ...
  const mandatoryTimeoutTeam = null;
  ...
  const fibaTimeoutState = (teamId) => {
    const timeoutsUsed = (game?.playByPlayActions || []).filter((action) => (
      action.actionType === "timeout"
      && action.teamId === teamId
    ));
    const firstHalfUsed = timeoutsUsed.filter((action) => action.period <= 2).length;
    const secondHalfUsed = timeoutsUsed.filter((action) => action.period >= 3 && action.period <= 4).length;
    const overtimeUsed = timeoutsUsed.filter((action) => action.period >= 5).length;
    if (currentPeriod <= 2) return { remaining: Math.max(0, 2 - firstHalfUsed), total: 2 };
    if (currentPeriod <= 4) return { remaining: Math.max(0, 3 - secondHalfUsed), total: 3 };
    return { remaining: Math.max(0, 1 - overtimeUsed), total: 1 };
  };
  const awayTimeoutState = isPregame ? { remaining: 2, total: 2 } : fibaTimeoutState(awayTeamId);
  const homeTimeoutState = isPregame ? { remaining: 2, total: 2 } : fibaTimeoutState(homeTeamId);
  const awayTimeoutsRemaining = awayTimeoutState.remaining;
  const homeTimeoutsRemaining = homeTimeoutState.remaining;
  const isGLeagueGame = awayLeague === "gleague" || homeLeague === "gleague";
  const awayResetUsed = false;
  const homeResetUsed = false;
      ...
      {showExtras && (
        <>
          ...
          {strategyHistoryModalOpen ? (
            <div className={styles.noteOverlay} onClick={closeStrategyHistoryModal}>
              ...
              >
                ...
                <div className={styles.strategyFeedbackSubtitle}>Saved strategy updates from Q4/OT situations tracked in the final 6:00.</div>
          ...
          <LateGameMatrixPanel
            ...
            onToggleCollapsed={toggleStrategyPanel}
            awayTeam={awayTeam}
            homeTeam={homeTeam}
            strategyState={strategyState}
            strategyEvaluation={strategyEvaluation}
            strategyVantageTeamId={strategyVantageTeamId}
            setStrategyVantageTeamId={setStrategyVantageTeamId}
            strategyOverrides={strategyOverrides}
            setStrategyOverrides={setStrategyOverrides}
            strategyManualOpen={strategyManualOpen}
            setStrategyManualOpen={setStrategyManualOpen}
            strategyOverrideDraft={strategyOverrideDraft}
