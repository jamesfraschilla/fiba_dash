...
export default function Minutes() {
  ...
  const homePlayers = Array.isArray(data?.playerTotals?.home) ? data.playerTotals.home : [];
  const awayPlayers = Array.isArray(data?.playerTotals?.away) ? data.playerTotals.away : [];
    ...
    return (
      <div className={styles.container}>
        ...
        <div className={styles.stateMessage}>
          Substitution-based stint data is not available from the current FIBA feed. Showing box score minutes only.
        </div>
        <section className={styles.periods}>
          <div className={styles.period}>
            <div className={styles.periodTitle}>Player Minutes</div>
            <div className={styles.stintRow}>
              <div className={styles.teamLabels}>
                <div className={styles.teamLabel}>
                  <img className={styles.teamLabelLogo} src={teamLogoUrl(awayTeam.teamId)} alt={`${awayTeam.teamName} logo`} />
                </div>
                <div className={styles.teamLabel}>
                  <img className={styles.teamLabelLogo} src={teamLogoUrl(homeTeam.teamId)} alt={`${homeTeam.teamName} logo`} />
                </div>
              </div>
              <div className={styles.stintsContainer}>
                <div className={styles.stintCell}>
                  <div className={styles.playersSection}>
                    {awayPlayers.map((player) => (
                      <div key={player.personId || player.fullName}>
                        {player.fullName || `${player.firstName || ""} ${player.familyName || ""}`.trim()} ({formatMinutes(player.minutes || "PT0M0.00S")})
                      </div>
                    ))}
                  </div>
                  <div className={styles.playersSection}>
