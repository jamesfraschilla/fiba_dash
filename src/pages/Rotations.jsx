...
const REGULATION_PERIOD_MINUTES = 10;
const MINUTES = Array.from({ length: REGULATION_PERIOD_MINUTES }, (_, index) => REGULATION_PERIOD_MINUTES - index);
...
function buildGenericRotationsConfig(team) {
  const teamId = String(team?.teamId || "").trim();
  if (!teamId) return null;
  const displayName = String(team?.teamName || team?.fullName || team?.teamTricode || "Team").trim();
  return {
    key: `team:${teamId}`,
    label: String(team?.teamTricode || displayName).toUpperCase(),
    defaultPlayers: Array.from({ length: 17 }, (_, index) => ({
      id: `p${index + 1}`,
      name: "",
      cap: 40,
...
function getRotationsScopeForGame(game) {
  ...
  return (
    getRotationsTeamConfig(game?.homeTeam)
    || getRotationsTeamConfig(game?.awayTeam)
    || buildGenericRotationsConfig(game?.homeTeam)
    || buildGenericRotationsConfig(game?.awayTeam)
  );
}

function getMonitoredGameTeam(game, teamScope) {
  if (!game || !teamScope) return null;
  if (teamScope.startsWith("team:")) {
    const scopeTeamId = teamScope.slice(5);
  ...
  if (isWashingtonScope) {
    if (TEAM_ROTATIONS_CONFIG.washington.matches(game?.homeTeam)) return game.homeTeam;
    if (TEAM_ROTATIONS_CONFIG.washington.matches(game?.awayTeam)) return game.awayTeam;
    return null;
  }
  if (TEAM_ROTATIONS_CONFIG.capital_city.matches(game?.homeTeam)) return game.homeTeam;
  if (TEAM_ROTATIONS_CONFIG.capital_city.matches(game?.awayTeam)) return game.awayTeam;
  return null;
}
...
const createDefaultPlayers = (teamScope = "washington") => (
  ((teamScope.startsWith("team:") ? buildGenericRotationsConfig({ teamId: teamScope.slice(5), teamTricode: "TEAM" }) : TEAM_ROTATIONS_CONFIG[teamScope])?.defaultPlayers || TEAM_ROTATIONS_CONFIG.washington.defaultPlayers)
    .map((player) => ({ ...player }))
);

const createDefaultDepthChart = (teamScope = "washington") => (
  ((teamScope.startsWith("team:") ? buildGenericRotationsConfig({ teamId: teamScope.slice(5), teamTricode: "TEAM" }) : TEAM_ROTATIONS_CONFIG[teamScope])?.defaultDepthRows || TEAM_ROTATIONS_CONFIG.washington.defaultDepthRows)
    .map((row) => row.slice())
);
    ...
    <section class="pdf-page ${side}">
      ...
      <div class="pdf-column">
        ...
        <div class="pdf-logo-wrap">
          <img class="pdf-logo" src="${escapeHtml(logoUrl)}" alt="Team logo" />
        </div>
      </div>
    </section>
  `;
...
export default function Rotations() {
  ...
  const monitoredGameTeam = useMemo(
    () => getMonitoredGameTeam(game, monitoredTeamScope),
  ...
  const exportHeaderLine = useMemo(
    () => `${monitoredGameTeam?.teamTricode || monitoredTeam?.label || "TEAM"} ${opponentLine}`,
    [monitoredGameTeam, monitoredTeam, opponentLine]
  );
  ...
  const handleExportPdf = async () => {
    ...
    const fallbackLogoUrl = new URL("../assets/WWizards_Primary_Icon.png", import.meta.url).href;
    const logoUrl = monitoredGameTeam?.teamId ? teamLogoUrl(monitoredGameTeam.teamId) : fallbackLogoUrl;
    const fontResponse = await fetch(fontUrl);
    const fontBytes = await fontResponse.arrayBuffer();
    let logoResponse = await fetch(logoUrl);
    if (!logoResponse.ok && logoUrl !== fallbackLogoUrl) {
  ...
  if (!rotationsAvailable) {
    return (
      <div className={`${styles.page} ${isDepthCellPressActive ? styles.pageSelectionLock : ""}`}>
        ...
        <div className={styles.stateMessage}>Rotations is unavailable for this game.</div>
      </div>
    );
  }
